...
function Sonnensystem.new ()  
  ...
  instance.sonne  = Himmelskoerper.new("Sonne",   1.989*10^30,  1391000/2,{0*10^6,    0, 0},  {0,0,0},        color.sunny_yellow)
...
function Sonnensystem:tick()
  for n1, h1 in pairs(self) do
    for n2, h2 in pairs(self) do
      ...
      dv = a  * 3600 * 3,6 -- v = a * t (3600s) in km/h (*3,6)

      -- Berechne Richtungsvektor als Einheitsvektor
      v0r = {1/r*(h1.position[1] - h2.position[1]),
             1/r*(h1.position[2] - h2.position[2]),
             1/r*(h1.position[3] - h2.position[3])}
      
     -- Multipliziere die Geschwindigkeiten in den Einheitsvektor
      h2.geschwindigkeit = {h2.geschwindigkeit[1] + v0r[1] * dv,
                            h2.geschwindigkeit[2] + v0r[2] * dv,
                            h2.geschwindigkeit[3] + v0r[3] * dv}

    ::continue::
    end
  end

  -- Berechne neue position aus der Geschwindigkeit pro Stunde
